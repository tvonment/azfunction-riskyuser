...
const MINUTES_RANGE = +process.env["APP_TIMERANGE_MIN"] || 5;
...
const timerTrigger: AzureFunction = async function (context: Context, myTimer: any): Promise<void> {
    ...
    riskdetections = riskdetections.filter(isFromLastMinutes)
...
function isFromLastMinutes(riskd: RiskDetections) {

    console.log(riskd.id)
    let aDT = new Date(riskd.activityDateTime).getTime();
    let timeStamp = Math.round(new Date().getTime() / 1000);
    let timeStampago = timeStamp - (MINUTES_RANGE * 60);
    let isinrange = aDT >= new Date(timeStampago*1000).getTime();
...
async function sendSupportMail(token: string, detections: RiskDetections[]) {
    ...
    let data = {
        "message": {
          "subject": "Microsoft Azure Risk Detections Alert",
          "body": {
            "contentType": "Text",
...
function getEmailText(detections: RiskDetections[]): string {
    ...
    detections.forEach(detection => {
        ...
        text += "Mail: " + detection.userPrincipalName + "\n";
